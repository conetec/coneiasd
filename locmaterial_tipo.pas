...
type
  Tfrmlocmaterial_tipo = class(TForm)
    Label3: TLabel;
    DBGrid1: TDBGrid;
    sair: TBitBtn;
    Edit1: TEdit;
...
var
  frmlocmaterial_tipo: Tfrmlocmaterial_tipo;
...
procedure Tfrmlocmaterial_tipo.Edit1Exit(Sender: TObject);
begin
  edit1.text:='';
end;

procedure Tfrmlocmaterial_tipo.sairClick(Sender: TObject);
begin
  frmlocmaterial_tipo.close;
end;

procedure Tfrmlocmaterial_tipo.op1Click(Sender: TObject);
begin
 edit2.SetFocus;
end;

procedure Tfrmlocmaterial_tipo.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocmaterial_tipo.BitBtn1Click(Sender: TObject);
var
  x,a:string;
begin
  with data10.myprocura do begin
    close;
    sql.clear;
    x:='select Codigo,descricao,ativo from material_tipo ';
    x:=x+' where (';
    ...
    sql.add(x);
    open;
    frmlocmaterial_tipo.Caption:='Categorias  = '+inttostr(recordcount);
  end;
  edit2.Text:='';
  dbgrid1.SetFocus;
end;

procedure Tfrmlocmaterial_tipo.Edit3Exit(Sender: TObject);
begin
...
procedure Tfrmlocmaterial_tipo.FormShow(Sender: TObject);
var
  i:integer;
begin
...
procedure Tfrmlocmaterial_tipo.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 Then Begin SelectNext(Sender as tWinControl, True, True ); Key := #0; end;
end;

procedure Tfrmlocmaterial_tipo.DBGrid1TitleClick(Column: TColumn);
Var
  i, iTemp : Integer;
begin
  i := 0;
...
procedure Tfrmlocmaterial_tipo.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocmaterial_tipo.FormClose(Sender: TObject;
