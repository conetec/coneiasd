...
procedure Tfrmlocestudob.Edit1Exit(Sender: TObject);
begin
  edit1.text:='';
end;

procedure Tfrmlocestudob.sairClick(Sender: TObject);
begin
  frmlocestudob.close;
end;

procedure Tfrmlocestudob.op1Click(Sender: TObject);
begin
 edit2.SetFocus;
end;

procedure Tfrmlocestudob.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocestudob.BitBtn1Click(Sender: TObject);
var
  x,a:string;
begin
  with data10.myprocura do begin
    close;
    sql.clear;
    x:='select Codigo,descricao,ativo from material ';
    x:=x+' where (';
    x:=x+' (igreja='+#39+frmprincipal.Configura[3]+#39+') and ';
    x:=x+' (categoria='+#39+frmprincipal.Configura[9]+#39+') and ';
    ...
    sql.add(x);
    open;
    frmlocestudob.Caption:='Estudos Bíblicos = '+inttostr(recordcount);
  end;
  edit2.Text:='';
  dbgrid1.SetFocus;
end;

procedure Tfrmlocestudob.Edit3Exit(Sender: TObject);
begin
...
procedure Tfrmlocestudob.FormShow(Sender: TObject);
var
  i:integer;
begin
...
procedure Tfrmlocestudob.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 Then Begin SelectNext(Sender as tWinControl, True, True ); Key := #0; end;
end;

procedure Tfrmlocestudob.DBGrid1TitleClick(Column: TColumn);
Var
  i, iTemp : Integer;
begin
  i := 0;
...
procedure Tfrmlocestudob.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocestudob.FormClose(Sender: TObject;
