...
var
  frmlocprofissao: Tfrmlocprofissao;
...
procedure Tfrmlocprofissao.Edit1Exit(Sender: TObject);
begin
  edit1.text:='';
end;

procedure Tfrmlocprofissao.sairClick(Sender: TObject);
begin
  frmlocprofissao.close;
end;

procedure Tfrmlocprofissao.op1Click(Sender: TObject);
begin
 edit2.SetFocus;
end;

procedure Tfrmlocprofissao.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocprofissao.BitBtn1Click(Sender: TObject);
var
  x,a:string;
begin
  with data10.myprocura do begin
    close;
    sql.clear;
    x:='select Codigo,descricao,ativo from profissao ';
    x:=x+' where (';
    ...
    sql.add(x);
    open;
    frmlocprofissao.Caption:='Profissões = '+inttostr(recordcount);
  end;
  edit2.Text:='';
  dbgrid1.SetFocus;
end;

procedure Tfrmlocprofissao.Edit3Exit(Sender: TObject);
begin
...
procedure Tfrmlocprofissao.FormShow(Sender: TObject);
var
  i:integer;
begin
...
procedure Tfrmlocprofissao.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 Then Begin SelectNext(Sender as tWinControl, True, True ); Key := #0; end;
end;

procedure Tfrmlocprofissao.DBGrid1TitleClick(Column: TColumn);
Var
  i, iTemp : Integer;
begin
  i := 0;
...
procedure Tfrmlocprofissao.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocprofissao.FormClose(Sender: TObject;
