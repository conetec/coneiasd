...
type
  Tfrmcadclassebiblica = class(TForm)
    BitBtn1: TBitBtn;
    BitBtn2: TBitBtn;
    Label1: TLabel;
    Label4: TLabel;
...
var
  frmcadclassebiblica: Tfrmcadclassebiblica;
...
procedure Tfrmcadclassebiblica.limpar;
begin
  edit1.Text:='0';
...
procedure Tfrmcadclassebiblica.mostrar;
var
  i:integer;
begin
...
procedure Tfrmcadclassebiblica.FormActivate(Sender: TObject);
begin
  if (frmcadclassebiblica.tag=1) or (frmcadclassebiblica.tag=3) then begin
    frmcadclassebiblica.caption:='Inclusão de Pequeno Grupo';
    limpar;
    edit1.SetFocus;
  end;
  if frmcadclassebiblica.tag=2 then begin
    frmcadclassebiblica.caption:='Alteração de Pequeno Grupo';
    limpar;
    mostrar;
    edit1.SetFocus;
  end;
...
procedure Tfrmcadclassebiblica.BitBtn2Click(Sender: TObject);
begin
  frmcadclassebiblica.close;
end;

procedure Tfrmcadclassebiblica.BitBtn1Click(Sender: TObject);
var
...
begin
  ...
  if ((frmcadclassebiblica.tag=1) or (frmcadclassebiblica.tag=3)) and (a=0) then begin
    with data10.exemysql do begin
      sql.clear;
      x:='INSERT INTO classebiblica ';
    ...
    if (frmcadclassebiblica.tag=1) then begin
      limpar;
      edit1.SetFocus;
    end;
    if (frmcadclassebiblica.tag=3) then begin
      codcad.text:=voltauc('classebiblica','codigo','membro='+cdmembro.text,adataid.text,ahoraid.text,frmprincipal.Configura[1]);
      frmcadclassebiblica.close;
    end;
  end;

  if (frmcadclassebiblica.tag=2) and (a=0) then begin
    with data10.exemysql do begin
      sql.clear;
...
procedure Tfrmcadclassebiblica.Edit1KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 Then Begin SelectNext(Sender as tWinControl, True, True ); Key := #0; end;
end;

procedure Tfrmcadclassebiblica.Edit1Enter(Sender: TObject);
begin
  SelectNext(Sender as tWinControl, True, True );
end;

procedure Tfrmcadclassebiblica.cdmembroExit(Sender: TObject);
var
  ax:string;
begin
...
procedure Tfrmcadclassebiblica.CadastrarClassedeEscolaSabatina1Click(
  Sender: TObject);
begin
...
procedure Tfrmcadclassebiblica.LocalizarClasseES1Click(Sender: TObject);
begin
  application.CreateForm(Tfrmlocmembro, frmlocmembro);
  frmlocmembro.showmodal;
  cdmembro.Text:=frmlocmembro.ocod.text;
