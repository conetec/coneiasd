...
procedure Tfrmlocdupla.Edit1Exit(Sender: TObject);
begin
  edit1.text:='';
end;

procedure Tfrmlocdupla.sairClick(Sender: TObject);
begin
  frmlocdupla.close;
end;

procedure Tfrmlocdupla.op1Click(Sender: TObject);
begin
 edit2.SetFocus;
end;

procedure Tfrmlocdupla.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocdupla.BitBtn1Click(Sender: TObject);
var
  x,a:string;
begin
  with data10.myprocura do begin
    close;
    sql.clear;
    x:='select Codigo,descricao,ativo from dupla ';
    x:=x+' where (';
    ...
    sql.add(x);
    open;
    frmlocdupla.Caption:='Duplas Missionárias = '+inttostr(recordcount);
  end;
  edit2.Text:='';
  dbgrid1.SetFocus;
end;

procedure Tfrmlocdupla.Edit3Exit(Sender: TObject);
begin
...
procedure Tfrmlocdupla.FormShow(Sender: TObject);
var
  i:integer;
begin
...
procedure Tfrmlocdupla.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 Then Begin SelectNext(Sender as tWinControl, True, True ); Key := #0; end;
end;

procedure Tfrmlocdupla.DBGrid1TitleClick(Column: TColumn);
Var
  i, iTemp : Integer;
begin
  i := 0;
...
procedure Tfrmlocdupla.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmlocdupla.FormClose(Sender: TObject;
