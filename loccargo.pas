...
procedure Tfrmloccargo.Edit1Exit(Sender: TObject);
begin
  edit1.text:='';
end;

procedure Tfrmloccargo.sairClick(Sender: TObject);
begin
  frmloccargo.close;
end;

procedure Tfrmloccargo.op1Click(Sender: TObject);
begin
 edit2.SetFocus;
end;

procedure Tfrmloccargo.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmloccargo.BitBtn1Click(Sender: TObject);
var
  x,a:string;
begin
  with data10.myprocura do begin
    close;
    sql.clear;
    x:='select Codigo,descricao,ativo from cargo ';
    x:=x+' where (';
    ...
    sql.add(x);
    open;
    frmloccargo.Caption:='Cargos / Funções = '+inttostr(recordcount);
  end;
  edit2.Text:='';
  dbgrid1.SetFocus;
end;

procedure Tfrmloccargo.Edit3Exit(Sender: TObject);
begin
...
procedure Tfrmloccargo.FormShow(Sender: TObject);
var
  i:integer;
begin
...
procedure Tfrmloccargo.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 Then Begin SelectNext(Sender as tWinControl, True, True ); Key := #0; end;
end;

procedure Tfrmloccargo.DBGrid1TitleClick(Column: TColumn);
Var
  i, iTemp : Integer;
begin
  i := 0;
...
procedure Tfrmloccargo.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure Tfrmloccargo.FormClose(Sender: TObject;
