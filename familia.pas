...
begin
 ...
 frmfamilia.Caption:='Famílias = '+inttostr(data10.mytudo.recordcount);
end;
...
begin
  ...
  with data10.mytudo do begin
    ...
    sql.add(x);
    open;
    frmfamilia.Caption:='Famílias = '+inttostr(recordcount);
  end;
  edit2.Text:='';
  dbgrid1.SetFocus;
end;
...
var
  x,cod:string;
begin
   if (application.MessageBox('Deseja excluir a Família?','Atenção',mb_yesno)<>id_no) then begin
