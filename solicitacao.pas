...
uses cadsolicitacao, datamodule10 , principal;
...
procedure Tfrmsolicitacao.sairClick(Sender: TObject);
begin
  frmsolicitacao.close;
end;

procedure Tfrmsolicitacao.incluirClick(Sender: TObject);
begin
 frmcadsolicitacao.tag:=1;
 frmcadsolicitacao.showmodal;
 data10.mytudo.Refresh;
 frmsolicitacao.Caption:='Solicitações = '+inttostr(data10.mytudo.recordcount);
end;

procedure Tfrmsolicitacao.editarClick(Sender: TObject);
var
  x,ocod:string;
begin
...
procedure Tfrmsolicitacao.op1Click(Sender: TObject);
begin
 edit2.SetFocus;
end;

procedure Tfrmsolicitacao.Edit1KeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  with data10.mytudo do begin
    close;
    sql.clear;
    x:='select Codigo,descricao from solicitacao ';
    x:=x+' where (';
    ...
    sql.add(x);
    open;
    frmsolicitacao.Caption:='Solicitações = '+inttostr(recordcount);
  end;
  edit2.Text:='';
  dbgrid1.SetFocus;
end;

procedure Tfrmsolicitacao.Edit3Exit(Sender: TObject);
begin
...
procedure Tfrmsolicitacao.Edit2KeyPress(Sender: TObject; var Key: Char);
begin
  If Key = #13 Then Begin SelectNext(Sender as tWinControl, True, True ); Key := #0; end;
end;

procedure Tfrmsolicitacao.excluirClick(Sender: TObject);
var
  x,cod:string;
begin
   if (application.MessageBox('Deseja excluir a Profissão?','Atenção',mb_yesno)<>id_no) then begin
    excluir.Enabled:=false;
    if data10.mytudo.RecordCount>0 then begin
      cod:=data10.mytudo.fieldbyname('codigo').asstring;

      x:='delete from solicitacao where codigo='+#39+cod+#39;
...
procedure Tfrmsolicitacao.DBGrid1TitleClick(Column: TColumn);
Var
  i, iTemp : Integer;
begin
  i := 0;
...
procedure Tfrmsolicitacao.DBGrid1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
